...
public class CourseService {
    ...
    public void updateSegmentOrderAndDelete(Long courseId, List<Long> newOrderList) {
        ...
        // 3️⃣ 요청으로 들어온 orderNo 집합 (order 파라미터는 기존 orderNo 기준)
        Set<Integer> requestedOrderNos = newOrderList.stream()
                .map(Long::intValue)
                .collect(HashSet::new, Set::add, Set::addAll);

        // 4️⃣ 요청에 없는 orderNo를 가진 segment → 삭제 처리
        List<CourseSegment> toRemove = existingSegments.stream()
                .filter(segment -> segment.getOrderNo() == null || !requestedOrderNos.contains(segment.getOrderNo()))
                .toList();
        existingSegments.removeAll(toRemove); // orphanRemoval=true 적용

        // 5️⃣ 요청 순서(newOrderList)에 맞춰 orderNo 재부여
        int order = 1;
        for (Long oldOrderNo : newOrderList) {
            Integer targetOrderNo = oldOrderNo.intValue();
            for (CourseSegment segment : existingSegments) {
                if (Objects.equals(segment.getOrderNo(), targetOrderNo)) {
        ...
        // 6️⃣ @Transactional 로 자동 flush, 삭제는 orphanRemoval 처리
    }
    ...
    public void addSegmentFromPopup(Long courseId, String placeName) {
        ...
        PlaceDto detail = PlaceDto.fromSearchResult(
                placeSearchService.searchOne(placeName, sigunNm)
        );
