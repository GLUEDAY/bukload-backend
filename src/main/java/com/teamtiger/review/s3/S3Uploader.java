package com.teamtiger.review.s3;
...
public class S3Uploader {
    ...
    @Value("${app.s3.presigned-get-seconds:0}")
    private long presignedGetSeconds;
    ...
    private String buildAccessUrl(String key) {
        if (!publicBaseUrl.isBlank()) {
            ...
            return publicBaseUrl + "/" + encKey;
        }
        // presigned GET (비공개 버킷일 때), SDK v2는 S3Presigner 사용
        if (presignedGetSeconds > 0) {
            ...
            var presign = software.amazon.awssdk.services.s3.presigner.model.GetObjectPresignRequest.builder()
                    .signatureDuration(java.time.Duration.ofSeconds(presignedGetSeconds))
                    .getObjectRequest(getReq)
                    .build();
            return presigner.presignGetObject(presign).url().toString();
        }
        // 마지막 수단: s3 path-style (권장X)
